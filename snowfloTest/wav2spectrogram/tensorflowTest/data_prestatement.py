...
for (dirpath, dirnames, filenames) in os.walk(path):  # 상위 폴더
    for filename in filenames:
        ...
        if filename.endswith(tuple(formats_to_convert)):
            ...
            plt.imshow(mfcc)
            plt.savefig('D:/#2021_CAPSTONE/_DataSet/sound_test_data_pretreatment (spec)/spec_test/mfcc/' + filename.replace(".wav", "") + '.png',bbox_inches='tight', pad_inches=0)
            plt.close()  # plt.cla(), plt.clf(), plt.close()
            ...
            plt.savefig('D:/#2021_CAPSTONE/_DataSet/sound_test_data_pretreatment (spec)/spec_test/delta2_mfcc/' + filename.replace(".wav", "") + '.png',bbox_inches='tight', pad_inches=0)
            plt.close()  # plt.cla(), plt.clf(), plt.close()
